...
export default function TicketSummary({ ticketPrice }) {
    ...
    const effectiveValueType =
        pricingFromServer?.discountValueType ??
    ...
    const finalAmountInr = useMemo(() => {
        ...
        if (effectiveValueType === "PERCENTAGE") {
            const discounted =
                ticketPrice * (1 - effectiveDiscountPercent / 100);
            return Number(Math.max(discounted, 0).toFixed(2));
        }
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const { order, pricing } = response.data;
            ...
            const options = {
                ...
                description: "DevFest Kolkata 2025 Ticket Purchase",
                order_id: order.id,
    ...
    return (
        <div className="w-full max-w-sm border-2 border-white rounded-lg p-6">
            ...
            <div className="mb-6">
                ...
                <div className={`flex md:flex-row gap-2 ${isApplied ? "" : "flex-col"}`}>
                    <input
                        type="text"
                        value={discountCode}
                        onChange={(e) => {
